...
def ground(domain, problem):
    ...
    groundedTask = plan_constraints_manager.GroundedConstraintsTask(task, atoms, actions, axioms)
    G = groundedTask.goal
    ...
    for kind in pddl.constraints.KINDS:
        C += groundedTask.ground_constraints_dict[kind][plan_constraints_manager.CONSTRAINTS]
    return F, A, I, G, C
