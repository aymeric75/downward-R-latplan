...
(define (problem roverprob1234)
        ...
        (:objects general - lander colour - mode high_res - mode low_res - mode
         rover1 - rover rover0 - rover rover0store - store rover1store - store waypoint0 - waypoint waypoint1 -
        ...
        (:init (visible waypoint1 waypoint0) (visible waypoint0 waypoint1)
         ...
         (at rover1 waypoint3) (available rover1) (store_of rover1store rover1)
         (empty rover1store) (equipped_for_soil_analysis rover1)
...
(exists (?r - rover) (at ?r waypoint0))
))(:constraints (and
(forall (?w - waypoint) (sometime (not (exists (?r - rover) (not (at ?r ?w))))))
(always (exists (?w - waypoint) (and (not (= ?w waypoint0)) (forall (?r - rover) (not (have_rock_analysis ?r ?w))))))
(forall (?w - waypoint) (at-most-once (or (have_rock_analysis rover0 ?w) (have_soil_analysis rover0 ?w))))
(forall (?r - rover ?w - waypoint) (sometime-after (have_rock_analysis ?r ?w) (have_soil_analysis ?r ?w)))
)
))